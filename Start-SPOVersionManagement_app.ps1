...
# Entra ID App Registration, then delegates to the standard module.
#
# Usage (Certificate Thumbprint - cert must be in CurrentUser\My):
#   .\Start-SPOVersionManagement_app.ps1 -AdminUrl "https://contoso-admin.sharepoint.com" `
...
#       -Unattended
#
# Usage (with Graph Report CSV instead of Graph API):
#   .\Start-SPOVersionManagement_app.ps1 -AdminUrl "https://contoso-admin.sharepoint.com" `
#       -TenantId "..." -ClientId "..." -CertificateThumbprint "..." `
#       -GraphReportCSV "C:\SharePointSiteUsage.csv" -Unattended
#
...
Write-Host "  Entra ID App Authentication" -ForegroundColor Cyan
Write-Host "================================================================" -ForegroundColor Cyan
...
if (-not $TenantId) {
    Write-Error "TenantId is required. Provide via -TenantId parameter or configure in AppPaths.json under EntraIdApp.TenantId"
    exit 1
}
if (-not $ClientId) {
    Write-Error "ClientId is required. Provide via -ClientId parameter or configure in AppPaths.json under EntraIdApp.ClientId"
    exit 1
}
if (-not $CertificateThumbprint -and -not $CertificatePath) {
    Write-Error "Either -CertificateThumbprint or -CertificatePath is required for certificate-based authentication."
    exit 1
}
